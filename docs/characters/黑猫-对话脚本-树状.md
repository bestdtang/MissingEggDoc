...
> 〔系统注〕**3-A** 仅 **2-hub**【菜单】、起身前（`!BlackCat_Entered`）；需 `E10_ViewWhiteStone`。**3-B** 不进 hub——漫画后点黑猫走入口判定直进（见章首说明）；与 **3-A** 共用 `BlackCat_StoneRevealShown`。
...
> 〔系统注〕`BlackCat_CaseLineDone && BlackCat_MintFishLineDone && !BlackCat_Entered`。黑猫入口判定或 **2-B-C** / **2-D** 出口 → **2-E**。大黄激将正文见下（原大黄样章 **2-D** 迁此）。
